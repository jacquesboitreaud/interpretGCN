# -*- coding: utf-8 -*-
"""
Created on Fri Nov 22 14:58:17 2019

@author: jacqu

Visualise attention weights of edges / DGL
"""
...
def get_attention_map(g, src_nodes, dst_nodes, h):
    ...
    for i, src in enumerate(src_nodes.tolist()):
        for j, dst in enumerate(dst_nodes.tolist()):
            # Find all edges where our nodes are involved
            if not g.has_edge_between(src, dst):
                continue
            eid = g.edge_id(src, dst)
            weight[i][j] = g.edata['e'][eid].squeeze(-1).cpu().detach() # alpha_ij, attn weight

    weight = weight.transpose(0, 2)
    att = th.softmax(weight, -2) # sum of attn weights for edges incoming to a node = 1 
    return att.numpy()
