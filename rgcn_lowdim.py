...
Same model except Input RGCNLayer (~embedding lookup) gets one-dimensional embedding, so that 
it gives one feature / edge (for integrated gradients analysis).
...
class Model(nn.Module):
    ...
    def build_model(self):
        ...
        # input to hidden
        i2e = self.build_embedding_layer()
        self.layers.append(i2e)
        e2h = self.build_first_layer()
        self.layers.append(e2h)
        # hidden to hidden
    ...
    def build_embedding_layer(self):
        ...
        return RGCNLayer(self.atom_types*self.charges, 1, self.num_rels, self.num_bases,
